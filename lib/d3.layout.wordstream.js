    ...
    function buildBoxes(data){
        ...
        if (!seperate) {
            ...
            layers = layers.map(l => {
                ...
                a.x = size[0];
                m.push(a);
                a = JSON.parse(JSON.stringify(m[0]));
                a.x = 0;
                m.splice(0, 0, a);
                return m;
            });
            ...
            layers = topics.map( t=>{
                ...
                .offset(d3.stackOffsetSilhouette)
                (data);
                layer[0].min =  d3.min(layer, (d => d3.min(d, e => e[0])));
                layer[0].max =  d3.max(layer, (d => d3.max(d, e => e[1])));
                return layer[0];});
            layers[0].offset = 0;
            ...
            layers = layers.map((l,j) => {
                var m = l.map((e, i) => {
                    return {y0: scale(e[0] + l.offset),
                        y: (scale(e[1]+ l.offset) - scale(e[0]+ l.offset)),
                        x: (i * boxWidth) + (boxWidth >> 1),
                        data: e.data}
                });
                let a = JSON.parse(JSON.stringify(m[m.length - 1]));
                a.x = size[0];
                ...
                a.x = 0;
                m.splice(0, 0, a);
                m.min = d3.min(m,n=> n.y0);
                m.max = d3.max(m,n=> (n.y0 + n.y));
                m.offset = m.min+(m.max-m.min)/2;
                m.key = l.key;
                m.index = j;
                return m;
            });
        ...
        for(let i = 0; i < data.length; i   ++){       // add properies to each object
            boxes.topics.forEach((topic, z) =>{
                ...
                while (++di < n) {
                    ...
                    d.rotate = (~~(Math.random() * 4) - 2) * rotateCorner; // +++++++++++++++++++++++
